...
Ao se usar map(), ele tornará o elemento list em objeto map então se precisa converter novamente em list.
    ...
    Links para apresentação de map():
...
ValueNewT = list(map(int, ValueNewT)) # converte o que está na lista usando map em integer, e muda a conversao que map faz para o type "map" em "list".
##############
